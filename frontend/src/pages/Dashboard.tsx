          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {reminders.slice(0, 2).map(r => (
                <div key={r.id} className="flex flex-col sm:flex-row sm:items-center justify-between bg-white/5 rounded-lg px-4 py-3 border border-white/5 gap-3 sm:gap-0">
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    <Clock className={`h-4 w-4 flex-shrink-0 ${getUrgencyColor(r.date)}`} />
                    <span className="text-sm font-medium truncate">{r.provider || r.filename || r.category || "Document"}</span>
                  </div>
